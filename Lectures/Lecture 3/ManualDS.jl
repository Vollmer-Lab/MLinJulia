...
# ╔═╡ 2f759542-28a4-4790-9aab-f846b0ad9239
begin
	using GLMNet
	using RDatasets
	using MLBase
...
# ╔═╡ daf1fe0d-3f3d-4ece-ab86-30aea9703890
using AlgebraOfGraphics, CairoMakie

# ╔═╡ 02ec68c5-f872-4247-ac9f-5f0e20a49dc4
md"""
Modified from [Julia for Data Science](https://github.com/JuliaAcademy/DataScience/blob/main/06.%20Classification.ipynb) Julia Academy course delivered by Dr. Hudar Nasir.
"""

# ╔═╡ 26ce1520-5f57-4322-bd5a-3f022d6cec3e
md"""
## Classification
...
# ╔═╡ eb4ecea4-32e3-456a-ac83-3a233ccbfea8
findaccuracy(predictedvals,groundtruthvals) = sum(predictedvals.==groundtruthvals)/length(groundtruthvals)

# ╔═╡ d920c016-34df-43c1-b095-c82e56b69cc9
md"""
Get the data first
"""

# ╔═╡ 1d70b9e6-2e37-4cda-81ef-a197fe4909e0
iris = dataset("datasets", "iris")

# ╔═╡ d45344a6-b710-422f-bb84-94af01112db7
begin
	plt = data(iris) * mapping(:SepalLength,:SepalWidth, color=:Species)
	draw(plt)
end

# ╔═╡ 89f0d7b0-079e-4a12-874d-1bed1b475dd3
begin
	X = Matrix(iris[:,1:4])
	irislabels = iris[:,5]
end

# ╔═╡ ba8bcf8a-e735-4f60-aad0-14c29f0c037d
X

# ╔═╡ ec4cedb6-55e2-4919-baf5-0bce7a4f52b1
begin
	irislabelsmap = labelmap(irislabels)
	y = labelencode(irislabelsmap, irislabels)
end

# ╔═╡ 2662a6d8-aa96-4aa2-a035-f6bf5d601a64
md"""
...
# ╔═╡ f59ae64a-cf59-4ccb-9fb5-baddd245dc81
function perclass_splits(y,at)
    uids = unique(y)
    keepids = []
    for ui in uids
        curids = findall(y.==ui)
        rowids = randsubseq(MersenneTwister(111), curids, at) 
        push!(keepids,rowids...)
    end
    return keepids
end

# ╔═╡ 6ad32085-447f-4a01-9fab-bf4953a95394
randsubseq

# ╔═╡ 2facc20d-c56f-4e3d-a940-52b017e63afa
begin
	trainids = perclass_splits(y, 0.7)
	testids = setdiff(1:length(y),trainids)
end

# ╔═╡ 230a5eb0-d912-4cc0-ac64-7bb427052229
md"""
...
# ╔═╡ 5cf205ef-3bb9-4d93-9d6d-470c3d8f0e32
assign_class(predictedvalue) = argmin(abs.(predictedvalue .- [1,2,3]))

# ╔═╡ b2008356-c93f-43e1-bbd4-088de138376f
md"""
### 🟣 Method 1: Lasso
"""

# ╔═╡ cd303783-9d1e-44fb-ae8d-be62fd2d7b74
begin
	# choose the best lambda to predict with.
	lasso_path_ = glmnet(X[trainids,:], y[trainids])
	lasso_cv = glmnetcv(X[trainids,:], y[trainids])
	lasso_lambda = lasso_path_.lambda[argmin(lasso_cv.meanloss)]
	
	lasso_path = glmnet(X[trainids,:], y[trainids],lambda=[lasso_lambda]);
end

# ╔═╡ 019c7388-7184-4360-9a0a-7bff34d7108d
begin
	lasso_q = X[testids,:];
	lasso_predictions_ = GLMNet.predict(lasso_path, lasso_q)
end

# ╔═╡ 64e9a979-1575-484f-b95b-cf791e28ba6e
begin
	lasso_predictions = assign_class.(lasso_predictions_)
	findaccuracy(lasso_predictions,y[testids])
end

# ╔═╡ 870ebdcc-5061-474c-860b-724d35ac5cc3
md"""
### 🟣 Method 2: Ridge
We will use the same function but set alpha to zero.
"""

# ╔═╡ fafce5f0-be75-47f7-83b1-bce3c642f3ae
begin
	# choose the best lambda to predict with.
	ridge_path_ = glmnet(X[trainids,:], y[trainids],alpha=0);
	ridge_cv = glmnetcv(X[trainids,:], y[trainids],alpha=0)
	ridge_lambda = ridge_path_.lambda[argmin(ridge_cv.meanloss)]
	ridge_path = glmnet(X[trainids,:], y[trainids],alpha=0,lambda=[ridge_lambda]);
	ridge_q = X[testids,:];
	ridge_predictions_ = GLMNet.predict(ridge_path, ridge_q)
	ridge_predictions = assign_class.(ridge_predictions_)
	findaccuracy(ridge_predictions, y[testids])
end

# ╔═╡ 3935ef24-4c8d-4491-955c-0d568e5fe253
md"""
### 🟣 Method 3: Elastic Net
We will use the same function but set alpha to 0.5 (it's the combination of lasso and ridge).
"""

# ╔═╡ dbe7d238-c947-4eba-ad9a-6d48bc773a61
begin
	# choose the best lambda to predict with.
	elnet_path_ = glmnet(X[trainids,:], y[trainids],alpha=0.5);
	elnet_cv = glmnetcv(X[trainids,:], y[trainids],alpha=0.5)
	elnet_lambda = elnet_path_.lambda[argmin(elnet_cv.meanloss)]
	elnet_path = glmnet(X[trainids,:], y[trainids],alpha=0.5,lambda=[elnet_lambda]);
	elnet_q = X[testids,:];
	elnet_predictions_ = GLMNet.predict(elnet_path, elnet_q)
	elnet_predictions = assign_class.(elnet_predictions_)
	findaccuracy(elnet_predictions,y[testids])
end

# ╔═╡ 28419ff8-d083-4454-9b9d-ca0ca7d34c10
md"""
### 🟣 Method 4: Decision Trees
We will use the package `DecisionTree`
"""

# ╔═╡ 9e4065e4-524f-45a2-81d5-61e673eab0d0
begin
	max_depth_range = -1:3
	pruning_purity = 1.0
	dt_tuning_accuracies = [
		mean(
			DecisionTree.nfoldCV_tree(
				y[trainids], X[trainids, :], 3, pruning_purity, max_depth_, verbose=false
			)
		) for max_depth_ in max_depth_range
	]
	max_depth = max_depth_range[argmax(dt_tuning_accuracies)]
end

# ╔═╡ bdef4f71-b837-4f14-8453-49bdbf118bbf
begin
	n_subfeatures = 0 # all features are considered
	dt_model = DecisionTree.build_tree(
		y[trainids], X[trainids, :], n_subfeatures, max_depth
	)

	dt_predictions = DecisionTree.apply_tree(dt_model, X[testids, :])
	findaccuracy(dt_predictions, y[testids])
end

# ╔═╡ d1bf73ca-8f2c-49b8-a764-ff22e95e1c46
md"""
### 🟣 Method 5: Random Forests
The `RandomForestClassifier` is available through the `DecisionTree` package as well.
"""

# ╔═╡ 11480373-edbb-4858-a3e0-938c498a48d1
begin
	ntrees_range = 2:6
	rf_tuning_accuracies = [
		mean(
			DecisionTree.nfoldCV_forest(
				y[trainids], X[trainids, :], 3, n_subfeatures, ntrees_, verbose=false
			)
		) for ntrees_ in ntrees_range
	]
	ntrees = ntrees_range[argmax(rf_tuning_accuracies)]
end

# ╔═╡ ad02064d-442d-4c8d-b908-a4a4e519a3b2
begin
	rf_model = DecisionTree.build_tree(
		y[trainids], X[trainids, :], n_subfeatures, ntrees
	)

	rf_predictions = DecisionTree.apply_tree(rf_model, X[testids, :])
	findaccuracy(rf_predictions, y[testids])
end

# ╔═╡ 88fdde07-9f47-4371-b5fc-fad64ed81caf
md"""
### 🟣 Method 6: Using a Nearest Neighbor method
We will use the `NearestNeighbors` package here.
"""

# ╔═╡ 85374d75-b6f6-4829-a4a4-60bf8bd7bd98
begin
	kdtree = KDTree(X[trainids, :]')
end

# ╔═╡ a98f24bf-2e03-4afb-a515-e848cb8d6b50


# ╔═╡ 122ea4de-af47-45ca-85d0-1e210ed85d98
begin
	K_range = 2:5
	kdtree_accuracies = zeros(length(K_range))
	kdtree_predictions_ = Vector{Vector{Int}}(undef, length(K_range))
	for (ind, K) in enumerate(K_range)
		idxs, dists = knn(kdtree, X[testids, :]', K, true)
		c = y[trainids][hcat(idxs...)]
		possible_labels = map(i->counter(c[:,i]),1:size(c,2))
		kdtree_predictions_[ind] = map(i->parse(Int,string(string(argmax(possible_labels[i])))), 1:size(c, 2))
		kdtree_accuracies[ind] = findaccuracy(kdtree_predictions_[ind], y[testids])
	end
	best_ind = argmax(kdtree_accuracies)
	best_K = K_range[best_ind]
	kdtree_predictions = kdtree_predictions_[best_ind]
end

# ╔═╡ d740dd2a-7d8d-4a9b-b7e9-641bcfe856b0
md"""
### 🟣 Method 7: Support Vector Machines
We will use the `LIBSVM` package here.
"""

# ╔═╡ b2743ba5-033c-45dc-b899-cc4c11ae4e31
svm_model = svmtrain(X[trainids,:]', y[trainids])

# ╔═╡ 60d1039f-fdc0-4690-99a4-9fb996715ccb
begin
	svm_predictions, decision_values = svmpredict(svm_model, X[testids,:]')
	findaccuracy(svm_predictions, y[testids])
end

# ╔═╡ 7d308500-8453-4222-b882-a2a4db7bb2e7
md"""
Putting all the results together:
"""

# ╔═╡ c6cb550a-08bc-402f-8402-613e5ab76521
begin
	overall_accuracies = zeros(7)
	methods = ["lasso","ridge","EN", "DT", "RF","kNN", "SVM"]
	ytest = y[testids]
	overall_accuracies[1] = findaccuracy(lasso_predictions, ytest)
	overall_accuracies[2] = findaccuracy(ridge_predictions, ytest)
	overall_accuracies[3] = findaccuracy(elnet_predictions, ytest)
	overall_accuracies[4] = findaccuracy(dt_predictions, ytest)
	overall_accuracies[5] = findaccuracy(rf_predictions, ytest)
	overall_accuracies[6] = findaccuracy(kdtree_predictions, ytest)
	overall_accuracies[7] = findaccuracy(svm_predictions, ytest)
	hcat(methods, overall_accuracies)
end

# ╔═╡ f39a48ef-b0ca-4321-bca8-921fa96a35a7
md"""
# Finally...
...
# ╔═╡ 692acbd3-780f-4e76-b87a-0706e516026d
md"""
# 🥳 One cool finding
...
|---|---|
| lasso  |$(round(overall_accuracies[1], digits=4))|
| ridge  |$(round(overall_accuracies[2], digits=4))|
| EN     |$(round(overall_accuracies[3], digits=4))|
| DT     |$(round(overall_accuracies[4], digits=4))|
| RF     |$(round(overall_accuracies[5], digits=4))|
| kNN    |$(round(overall_accuracies[6], digits=4))|
| SVM    |$(round(overall_accuracies[7], digits=4))|
"""
